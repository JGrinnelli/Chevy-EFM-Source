...
double		CDi						= 0.03;				// Induced Drag coeff (Function of CL)
double		CDde					= 0.02;			    // Drag due to Elevator Deflection
...
void ed_fm_simulate(double dt)
{
	...
	V_freeStream =  sqrt(airspeed.x * airspeed.x + airspeed.y * airspeed.y + airspeed.z * airspeed.z) * mpstofps + (thrust_aval / 90);
	...
	alpha_limit		= limit(aoaRaw, -44, 44);
	...
	L_roll			= rollmoment(Clb, Cldadot, Clda, Cldr, q, meanChord, wingArea, latStickInput, aileron_DEG, rudder_DEG, wingSpan, betaRad, rollRate_rps, rho, u, aoaRad, Clr, V_freeStream, pedalInput);
	M_pitch			= pitchmoment(Cmalpha, Cmde, Cmq, Cmadot, q, meanChord, wingArea, longStickInput, elevator_DEG, rudder_DEG, wingSpan, betaRad, pitchRate_rps, rho, u, aoaRad, V_freeStream, pedalInput, alphadot);
...
void ed_fm_set_command (int command, float value)
{
	...
	if (command == inputs::Mixture)
	{
		fuelMixture = pow(-(0.7 * value + 0.3),2) + 1;
	}

	if (command == inputs::Prop)
	{
